 ...
 * `configure` method.
  * 所有可插拔授权者必须实现的顶级接口。 Kafka将在启动时读取`authorizer.class.name`的值，使用默认构造函数创建指定类的实例，并调用其*`configure`方法。
 *
